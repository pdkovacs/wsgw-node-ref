...
const sendMessage = async (
	...
	message: E2EMessage,
	wsConnIds: string[],
	discardConnId: (connId: string) => Promise<void>
): Promise<number> => {
...
export const createBulkMessageHandler = (params: ApiHandlerParams): Handler => {
	...
	return async (req, res) => {
		if (isUndefined(req.session.user?.userId)) {
			req.logger.info("req.session.user?.userId is undefined");
			throw new Error();
		}
		...
		let lastStatus: number = StatusCodes.INTERNAL_SERVER_ERROR;
		try {
			req.logger.debug("sending messages", { "msgCount": allMessages.length });
			for (const messageIn of allMessages) {
				lastStatus = await sendMessage(req, params.wsgwUrl, req.session.user?.userId, messageIn, [], async () => { });
			}
		} catch (err) {
